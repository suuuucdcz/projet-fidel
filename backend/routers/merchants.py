...
def get_merchant_settings(merchant_id: str):
    ...
    m_res = supabase.table("merchants").select("name, reward_threshold, reward_description, color_hex, logo_url, hero_url, program_name, points_label, phone, website, loyalty_type, tiers, cashback_rate").eq("id", merchant_id).execute()
    if not m_res.data:
